                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="m-0 font-weight-bold text-gray-100 text-center">Daftar Data Fasilitas</h3>
                            </div>
                            <div class="card-body">
                                <a href="../admin/tambah_fasilitas.php" class="btn btn-icon-split btn-primary btn-sm">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example" class="table table-striped table-bordered first" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php 
                                                $sql = mysqli_query($con, "SELECT * FROM fasilitas, sub_paket, paket WHERE fasilitas.kode_subPaket=sub_paket.kode_subPaket AND sub_paket.kode_paket=paket.kode_paket GROUP BY fasilitas.kode_subPaket");
                                                $no = 1;
                                                while ($tampil = mysqli_fetch_array($sql)) {
                                                    $kode_subPaket = $tampil['kode_subPaket'];
                                                    ?>
                                                        <tr>
                                                            <td><?php echo $no++; ?></td>
                                                            <td><?php echo $tampil['nama_paket'].' '.$tampil['nama_sub']; ?></td>
                                                            <td><?php echo $tampil['ketentuan']; ?></td>
                                                            <td><?php 
                                                                    $sqlSub = mysqli_query($con, "SELECT kode_fasilitas, nama_barang FROM fasilitas, barang WHERE fasilitas.kode_barang = barang.kode_barang AND kode_subPaket = '$kode_subPaket'");
                                                                    while ($tampilSub = mysqli_fetch_array($sqlSub)) {
                                                                        echo $tampilSub['nama_barang'].'<br>';
                                                                    }
                                                                ?> 
                                                            </td>
                                                            <td><?php 
                                                                    ...
                                                                    while ($tampilSub = mysqli_fetch_array($sqlSub)) {
                                                                        ?>
                                                                        <a class="float-center text-primary" href="../admin/ubah_fasilitas.php?kode_fasilitas=<?php echo $tampilSub['kode_fasilitas']; ?>">Ubah</a>

                                                                        <a class="mx-3 text-danger float-right" onclick="return confirm('Yakin Ingin Menghapus ?')" href="../admin/hapus_fasilitas.php?kode_fasilitas=<?php echo $tampilSub['kode_fasilitas']; ?>">Hapus</a><br>
                                                                        <?php
                                                                    }
                                                                ?> 
                                                            </td>
