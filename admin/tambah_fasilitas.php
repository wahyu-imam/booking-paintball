<?php
    ...
    $sql = mysqli_query($con, "SELECT MAX(kode_fasilitas) AS kode FROM fasilitas");
    $data = mysqli_fetch_array($sql);
    $kode = $data['kode'];
    $urutan = (int) substr($kode, 1, 3);
    $urutan++;
    $huruf = "F";
    $kode_fasilitas = $huruf . sprintf("%03s", $urutan);
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="container-fluid dashboard-content ">
                            ...
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item"><a href="../admin/tambah_fasilitas.php" class="breadcrumb-link">Tambah Fasilitas</a></li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="m-0 font-weight-bold text-gray-100 text-center">Tambah Data Fasilitas</h3>
                    </div>
                    <div class="card-body">
                        <form role="form" method="POST">
                            <div class="form-group">
                                <label for="kode_fasilitas" class="col-form-label">Kode Fasilitas</label>
                                <input id="kode_fasilitas" name="kode_fasilitas" type="text" class="form-control" value="<?php echo $kode_fasilitas ?>" readonly>
                            </div>
                            <div class="form-group">
                                <label for="kode_subPaket" class="col-form-label">Nama Paket</label>
                                <select id="kode_subPaket" name="kode_subPaket" type="text" class="form-control" onclick="">
                                    <?php 
                                        $sql = mysqli_query($con, 'SELECT * FROM `paket`, sub_paket WHERE paket.kode_paket=sub_paket.kode_paket');
                                        while ($tampil = mysqli_fetch_array($sql)) {
                                    ?>
                                        <option value="<?php echo $tampil['kode_subPaket']; ?>"><?php echo $tampil['nama_paket'].' '.$tampil['nama_sub'].' '.$tampil['ketentuan']; ?></option>
                                        <?php       
                                        }
                                        ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select id="kode_barang" name="kode_barang" type="text" class="form-control" onclick="">
                                    <?php 
                                        $sql = mysqli_query($con, 'SELECT * FROM barang');
                                        while ($tampil = mysqli_fetch_array($sql)) {
                                    ?>
                                        <option value="<?php echo $tampil['kode_barang']; ?>"><?php echo $tampil['nama_barang'] ?></option>
                                        <?php       
                                        }
                                        ?>
                                </select>
...
<?php
    if(isset($_POST['simpan']))
    {
        ...
        $simpan = mysqli_query($con, "INSERT INTO fasilitas(kode_fasilitas, kode_subPaket, kode_barang) VALUES ('$kode_fasilitas','$kode_subPaket','$kode_barang')");
